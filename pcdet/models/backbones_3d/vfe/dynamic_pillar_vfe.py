...
class MultiScalePillarFeatureNet(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_sizes, grid_sizes, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)

        self.num_filters = self.model_cfg.NUM_FILTERS

        self.singlePFEs = nn.ModuleList()
        for i in range(len(voxel_sizes)):
            self.singlePFEs.append(DynamicPillarVFESimple2D(model_cfg, num_point_features, voxel_sizes[i], grid_sizes[i], point_cloud_range, **kwargs))
        # self.singlePFEs.append(DynamicPillarVFESimple2D(model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs))

    def get_output_feature_dim(self):
        return self.num_filters[-1]  

    def forward(self, batch_dict):
        batch_dict['pillar_features_list'] = []
        batch_dict['pillar_coords_list'] = []

        for pfe in self.singlePFEs:
            temp_batch_dict = pfe(batch_dict)
            batch_dict['pillar_features_list'].append(temp_batch_dict['pillar_features'])
            batch_dict['pillar_coords_list'].append(temp_batch_dict['pillar_coords'])
