...
class PillarRes18BackBone8x(nn.Module):   # 使用稀疏卷积层处理点云数据
    def __init__(self, model_cfg, input_channels, grid_sizes, **kwargs):   # lyt
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)   # 1D归一化层
        self.sparse_shapes = grid_sizes  # grid_sizes[[1, 0]]   # lyt
        
        block = post_act_block   # 自定义的卷积函数
        dense_block = post_act_block_dense
    ...
    def forward(self, batch_dict):
        batch_dict['multi_scale_2d_features'] = {}
        for idx in range(len(batch_dict['pillar_features_list'])):
            pillar_features, pillar_coords = batch_dict['pillar_features_list'][idx], batch_dict['pillar_coords_list'][idx]
            batch_size = batch_dict['batch_size']
            input_sp_tensor = spconv.SparseConvTensor(  # 系数卷积   点特征和点坐标封装为张量
                features=pillar_features,
                indices=pillar_coords.int(),
                spatial_shape=self.sparse_shapes[idx][[1, 0]] ,  # lyt
                batch_size=batch_size
            )
        ...
        # })
        #     batch_dict['multi_scale_2d_features'] = {}   # lyt
            batch_dict['multi_scale_2d_features'].update({
                idx: {
                    'x_conv1': x_conv1,
                    'x_conv2': x_conv2,
                    'x_conv3': x_conv3,
