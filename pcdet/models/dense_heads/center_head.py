...
class CenterHeadLYT(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_sizes, point_cloud_range, voxel_sizes,
                 predict_boxes_when_training=True):
        super().__init__()
        self.model_cfg = model_cfg
        self.num_class = num_class
        ...
        self.heads = []
        for idx in range(len((self.grid_sizes))):
            cuda = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            self.heads.append(CenterHead(model_cfg, input_channels, num_class, class_names, grid_sizes[idx], point_cloud_range, voxel_sizes[idx], self.status, predict_boxes_when_training).to(cuda))

    def forward(self, data_dict):  
        if self.training:
            data_dict['rois'] = []
            data_dict['roi_scores'] = [] 
            data_dict['roi_labels'] = []
        # data_dict['has_class_labels'] = True

        data_dict['final_box_dicts'] = []
        for idx in range(len(self.grid_sizes)):
            self.heads[idx](data_dict, idx, self.training)
    ...
    def get_loss(self):  
        ...
        for idx in range(len(self.grid_sizes)):
            if (loss == None):
                loss, tb_dict = self.heads[idx].get_loss()
            else:
                loss1, tb_dict1 = self.heads[idx].get_loss()
                loss += loss1
                result = {}
                for key in tb_dict.keys() | tb_dict1.keys():
                    result[key] = tb_dict.get(key, 0) + tb_dict1.get(key, 0)
                tb_dict = result
        return loss, tb_dict